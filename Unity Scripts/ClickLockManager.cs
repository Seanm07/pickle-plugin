...
public class ClickLockManager : MonoBehaviour
{
    ...
    public void ShowClickLock(string message, bool showSkipAfterTime = true, bool closeAllPrevious = false, bool instantlyShowSkip = false, bool silentSkip = false, bool opaqueBackground = false) {
        if (closeAllPrevious) {
            if(wasAdMobBannerVisible && adMobBannerHideDepth <= 0)
                for(int i=0;i < adMobBannerHideDepth;i++)
                    AdMob_Manager.instance.ShowBannerAd(AdMob_Manager.instance.activeBannerRefId);
        ...
        if(skipOverlay != null) skipOverlay.SetActive(instantlyShowSkip);
        if(container != null) container.SetActive(true);
        
        isVisible = true;

        if (AdMob_Manager.instance != null && AdMob_Manager.instance.bannerWantedVisible[AdMob_Manager.instance.activeBannerRefId]) {
    ...
    public void HideClickLock() {
        ...
        if (activeClickLockCount <= 0) {
            if(skipOverlay != null) skipOverlay.SetActive(false);
            ...
            if(skipOverlay != null) skipOverlay.SetActive(newActiveClickLock.showSkipAfterTime && newActiveClickLock.visibleTime >= 20f);
        }
            ...
            foreach (ActiveClickLockStorage activeClickLock in activeClickLocks) {
                if (activeClickLock.showSkipAfterTime) {
                    ...
                    if (activeClickLock.visibleTime >= 20f)
                        if(skipOverlay != null) skipOverlay.SetActive(true);
                }
            }
        }
    }
