...
public class IABManager : MonoBehaviour {
    ...
    public static event Action OnConsumeAllItemsComplete;

    public static event Action<IABItem> OnIABPurchaseComplete;
    public static event Action<IABItem, int> OnIABPurchaseCompleteWithQuantity;
    public static event Action<IABItem, int, string> OnIABPurchaseCompleteWithTxnId;
    public static event Action<IABItem, int, string, string> OnIABPurchaseCompleteExtended;
    ...
    private void Awake() {
        ...
        UnityServices.InitializeAsync().ContinueWith(task => {
            ...
            // Continue with IAB init regardless of Unity Services state
    ...
    public void OnPurchaseRestoreComplete() {
        FirebaseAnalyticsManager.LogEvent(PickleEventCategory.PickleScripts.IAP_RESTORE_COMPLETE);
    ...
    public void OnPurchaseRestoreFailed(InitializationFailureReason failureReason, string errorMessage) {
        ...
        FirebaseAnalyticsManager.LogEvent(PickleEventCategory.PickleScripts.IAP_RESTORE_FAILED, failureReason.ToString());
    ...
    public void OnConsumeAllComplete() {
        OnConsumeAllItemsComplete?.Invoke();
    }
    ...
    public void OnPurchaseVerificationCompleted(IABItem item, int quantity, string originalTransactionId, string payload) {
        FirebaseAnalyticsManager.LogEvent(PickleEventCategory.PickleScripts.IAP_PURCHASE_COMPLETE, item.productId);

        OnIABPurchaseComplete?.Invoke(item);
        OnIABPurchaseCompleteWithQuantity?.Invoke(item, quantity);
        OnIABPurchaseCompleteWithTxnId?.Invoke(item, quantity, originalTransactionId);
        OnIABPurchaseCompleteExtended?.Invoke(item, quantity, originalTransactionId, payload);
    ...
    public void OnPurchaseVerificationFailed(string sku, string error) {
        ...
        FirebaseAnalyticsManager.LogEvent(PickleEventCategory.PickleScripts.IAP_PURCHASE_FAILED, "Validation Failed");
    ...
    public string ISO8601ToReadableString(string input, bool appendS = true, bool noNumberOne = true, bool noNumbers = false) {
        ...
        if (!string.IsNullOrEmpty(input)) {
            ...
            // End atleast 2 characters before the end, the duration/time is invalid without a number
            for (int charId = 0; charId + 1 < inputLength; charId++) {
